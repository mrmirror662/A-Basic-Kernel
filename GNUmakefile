...
.PHONY: run
run: dummaOS.iso
	qemu-system-x86_64 -M q35 -m 2G -cdrom dummaOS.iso -boot d

.PHONY: run-uefi
run-uefi: ovmf-x64 dummaOS.iso
	qemu-system-x86_64 -M q35 -m 2G -bios ovmf-x64/OVMF.fd -cdrom dummaOS.iso -boot d
...
dummaOS.iso: limine kernel
	...
	xorriso -as mkisofs -b limine-cd.bin \
		...
		iso_root -o dummaOS.iso
	limine/limine-deploy dummaOS.iso
	rm -rf iso_root
...
.PHONY: clean
clean:
	rm -rf iso_root dummaOS.iso barebones.hdd
	$(MAKE) -C kernel clean
